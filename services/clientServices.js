...
const searchNameService = function(name, callback){
    clientDAO.findName(name,function(err, rows){
        if (err) {
            throw err;
        }
        if (rows.length == 0) {
...
const deleteService = function(num_client, callback) {
    clientDAO.deleteClient(num_client, function(err, rows) {
        if (err) {
            throw err;
        } else {
            callback(false, rows);
